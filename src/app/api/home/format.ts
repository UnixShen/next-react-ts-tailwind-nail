...
export const formatHomeData = (today: AddFormValues[], month: AddFormValues[]): HomeDataItem[] => {
    ...
    const monthReturn = month.filter((item) => item.type === ADD_TYPE_VALUE.ADD_EXPENSE).reduce((acc, cur) => acc + (cur.amount || 0), 0);
    ...
    return [
        ...
        },
        {
            ...
            title: "今日提成",
            amount: Number(((todayIncome - todayReturn) / 2).toFixed(2)),
        },
        {
            type: ADD_TYPE_VALUE.ADD_INCOME,
            title: "本月总提成",
            amount: Number(((monthIncome - monthReturn) / 2).toFixed(2)),
        },
